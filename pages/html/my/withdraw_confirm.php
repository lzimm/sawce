...
<form action="<?=build_link('my','withdraw', 'complete', $reqstring)?>" method="post" class="makeform" id="sw_ajax">

<table class="makebasic rowlines" table cellpadding="0" cellspacing="0">
	<tr class="first_row">
		<td width="20%"><label>Current Balance</label>$<?=$user->_balance;?></td>
		<td width="40%"><label>PayPal Account</label><?=$email->get();?></td>
		<td width="40%"><label>Withdraw Credits</label>$<?=$credits->get();?></td>
	</tr>
	...
	<tr>
		<td width="20%"><?=$email->build();?><?=$credits->build();?></td>
