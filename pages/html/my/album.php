...
<form action="<?=build_link('my','album', $album->_id, 'upload')?>" method="post" enctype="multipart/form-data" class="makeform">

<table class="makebasic" cellpadding="0" cellspacing="0">
	<tr>
		<td width="40%" class="first"><label>Song Name</label><?=$song_name->build();?></td>
		<td width="40%"><label>File</label><?=$song->build();?><?=$song_album->build();?></td>
		<td width="20%"><label>Upload</label><?=$song_submit->build();?></td>
	</tr>
</table>
