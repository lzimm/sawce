...
SWFUpload.prototype.initSettings = function (init_settings) {
    ...
    // Flags
    this.addSetting("begin_upload_on_queue",  init_settings.begin_upload_on_queue,  false);
    this.addSetting("use_server_data_event",  init_settings.use_server_data_event,  true);
    ...
    this.addSetting("flash_color",        init_settings.flash_color,        "#666666");
	...
	// Sawce Custom
	this.container     	 = this.retrieveSetting(init_settings.container,            '');
};
...
SWFUpload.prototype.loadFlash = function () {
	...
	//$('#object_container').append(this.getFlashHTML());
};
...
SWFUpload.prototype.startUpload = function (onComplete, form, file_id) {
	
	// Sawce Custom
	this.afterComplete = onComplete;
	this.parentForm = form;
	$('#div_' + this.container).find('.upload_cancel').hide();
...
SWFUpload.prototype.cancelQueue = function () {
	// Sawce Custom
	this.queued = false;
	$('#div_' + this.container).find('.upload_browse').show();
	$('#div_' + this.container).find('.upload_cancel').hide();
	$('#div_' + this.container).find('.upload_file').hide();
	$('#div_' + this.container).find('.upload_file').find('span').html('');
...
SWFUpload.prototype.setUseServerDataEvent = function (use_server_data_event) {
    var movie_element = this.getMovieElement();
    if (movie_element !== null && typeof(movie_element.SetUseServerDataEventEvent) === "function") {
        try {
...
SWFUpload.prototype.fileQueued = function (file) {
	...
	$('#div_' + this.container).parent().removeClass('file_error');

	// Sawce Custom
	this.queued = true;
	$('#div_' + this.container).find('.upload_browse').hide();
	$('#div_' + this.container).find('.upload_cancel').show();
	$('#div_' + this.container).find('.upload_file').show();
	$('#div_' + this.container).find('.upload_file').find('span').html(file.name);	
};
...
SWFUpload.prototype.fileProgress = function (file, bytes_complete, bytes_total) {
    this.debugMessage("File Progress: " + file.id + ", Bytes: " + bytes_complete);

	// Sawce Custom
	var percent = bytes_complete/bytes_total*100;
	$('#div_' + this.container).find('.progress').css('width', percent + '%');
};
...
SWFUpload.prototype.fileComplete = function (file, server_data) {
	...
	// Sawce Custom
	$("#div_" + this.container).find('input').val(server_data);
};
...
SWFUpload.prototype.queueComplete = function (file_upload_count) {
	...
	// Sawce Custom
	$("#div_" + this.container).find('.progress').css('width', '100%');
...
// Sawce Custom
SWFUpload.prototype.parentSubmit = function() {
	if (this.parentForm) {
